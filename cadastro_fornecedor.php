<!-- 3ª Digitação (Aqui) -->
<?php
// Inclui o arquivo que valida a sessao do usuário
...
function redimensionarESalvarImagem($arquivo, $largura = 80, $altura = 80) {
    ...
    $check = getimagessize($arquivo["tmp_name"]);
        if ($check === false) {
        ...
        if ($tipo_arquivo == "jpg" || $tipo_arquivo == "jpeg") {
            ...
            $image_original = imagecreatefrompng($arquivo["tmp_name"]);
        ...
        $ratio = min($largura / $largura_original, $altura / $altura_origina);
        ...
        // Redmensiona a imagem original para a nova imagem
        imagecopyresampled($nova_imagem, $imagem_original, 0, 0, 0, 0, $nova_largura, $nova_altura,
